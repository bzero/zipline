...
def prev_bar(spec, env=None):
    """
    Returns a function that will return the previous bar for a given datetime.
    """
    if spec.frequency.unit_str == 'd':
        if spec.frequency.data_frequency == 'minute':
            ...
            return env.previous_trading_day
    else:
        return env.previous_market_minute
...
def _to_midnight(arr):
    """
    Normalize the dates of all the values of an array of datetime64s.
    """
    return np.array(
        list(
            map(
                lambda dt: pd.tslib.normalize_date(
                    pd.Timestamp(dt, 'utc'),
                ).value,
                arr
            ),
...
def _extract_values(arr):
    """
    call .value on each element of arr.
    """
    return np.array(
        list(map(lambda dt: dt.value, arr)),
        dtype='datetime64[ns]',
    )
...
class HistoryContainer(object):
    ...
    def __init__(self,
        ...
        self.buffer_panel = self.create_buffer_panel(initial_dt, before_init)

        # Dictionaries with Frequency objects as keys.
        self.digest_panels, self.cur_window_starts, self.cur_window_closes = \
            self.create_digest_panels(initial_sids, initial_dt, before_init)
